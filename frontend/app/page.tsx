      ...
      <div style={{ background: "#fff", padding: "12px 16px 0", position: "sticky", top: 0, zIndex: 40, boxShadow: "0 1px 0 #E5E5EA" }}>
        <div style={{ display: "flex", alignItems: "center", justifyContent: "space-between", marginBottom: 10 }}>
          <div style={{ display: "flex", alignItems: "center", gap: 10 }}>
            <div style={{ width: 34, height: 34, background: "#F5C518", borderRadius: 10, display: "flex", alignItems: "center", justifyContent: "center", fontSize: 18 }}>
              🏠
            </div>
          ...
          <div style={{ display: "flex", gap: 8 }}>
            <Link href="/top10" style={{ width: 36, height: 36, background: "#000", borderRadius: "50%", display: "flex", alignItems: "center", justifyContent: "center", textDecoration: "none" }}>
              <span style={{ fontSize: 9, fontWeight: 900, color: "#F5C518", lineHeight: 1.1, textAlign: "center" }}>TOP10</span>
            </Link>
        ...
        {view === "gallery" && (
          <div style={{ display: "flex", gap: 8, marginBottom: 10 }}>
            <div style={{ position: "relative", flex: 1 }}>
              ...
              <input className="jm-input" style={{ paddingLeft: 38, paddingTop: 11, paddingBottom: 11, fontSize: 14 }}
                placeholder="Nimani qidiramiz?" value={search} onChange={(e) => setSearch(e.target.value)} />
            </div>
      ...
      {view === "map" && (
        <div style={{ height: "calc(100vh - 130px)", position: "relative" }}>
          ...
          <div style={{ position: "absolute", top: 10, left: 10, right: 10, display: "flex", gap: 6 }}>
            {["sale", "rent", "kunlik"].map((d) => (
              <button key={d} onClick={() => setDeal(d)}
                style={{ padding: "8px 14px", borderRadius: 20, border: "none", cursor: "pointer", fontWeight: 700, fontSize: 13, background: deal === d ? "#000" : "#fff", color: deal === d ? "#fff" : "#000", boxShadow: "0 2px 8px rgba(0,0,0,.15)" }}>
          ...
          <button onClick={async () => {
            if (!userPos) return;
            try {
              const { data } = await api.get("/api/listings/near", { params: { lat: userPos[0], lng: userPos[1], radius_km: 50, deal_type: deal } });
              setListings(data);
            } catch {}
            ...
            <>
              ...
              <div style={{ display: "flex", alignItems: "center", justifyContent: "space-between", marginBottom: 8 }}>
                <h2 style={{ fontSize: 18, fontWeight: 800 }}>
                  Biz <span style={{ color: "#F5C518" }}>{filtered.length.toLocaleString()}</span> ta e&apos;lon topdik
                </h2>
        ...
        <>
          ...
          <div className="sheet">
            ...
            <div style={{ padding: "0 20px" }}>
              ...
              <button className="yellow-btn" style={{ width: "100%", padding: 16, fontSize: 16 }} onClick={() => setShowFilter(false)}>
                Qo&apos;llash
              </button>
              <button style={{ width: "100%", padding: "14px", background: "none", border: "none", cursor: "pointer", fontSize: 15, color: "#555", fontWeight: 600 }} onClick={() => setShowFilter(false)}>
                Keyingi
              </button>
            </div>
          </div>
